...
const mockBonjourFind = vi.fn(
  (_opts: BrowserConfig | null, onup?: (service: Service) => void) => {
    if (onup) {
...
const mockBonjourFindOne = vi.fn(
  (
...
describe("Adapter", () => {
  ...
  it("finds all devices", async () => {
    vi.spyOn(SerialPort, "list").mockResolvedValueOnce([
      Object.assign(
        { pnpId: "deconz conbee ii", serialNumber: "", locationId: "" },
        DECONZ_CONBEE_II,
      ),
      Object.assign(
        { pnpId: "zbdongle-e", serialNumber: "", locationId: "" },
        EMBER_ZBDONGLE_E,
      ),
      Object.assign(
        { pnpId: "cc2538", serialNumber: "", locationId: "" },
        ZSTACK_CC2538,
      ),
      Object.assign(
        { pnpId: "nordic", serialNumber: "", locationId: "" },
        ZBOSS_NORDIC,
      ),
      Object.assign(
        {
          pnpId: "zigate-plus-v2",
          serialNumber: "",
          locationId: "",
          manufacturer: "",
        },
      ...
      Object.assign(
        { pnpId: "zbt-2", serialNumber: "", locationId: "" },
        ZBT_2,
      ),
    ]);
      ...
      Object.assign(
        {
          ...
          serialNumber: "",
          locationId: "",
          friendlyName: "silicon labs cp210x",
        },
        EMBER_ZBDONGLE_E_CP,
      ),
    ]);
  ...
  describe("mDNS discovery", () => {
    ...
    it("times out", async () => {
      ...
      await expect(
        Adapter.create(
          { panID: 0, channelList: [] },
          { path: `mdns://${fakeAdapterName}` },
          "test.db",
          { disableLED: false },
        ),
      ).rejects.toThrow(
        `Coordinator [${fakeAdapterName}] not found after timeout of 2000ms!`,
      );
    });
  ...
  describe("TCP discovery", () => {
    ...
    it.each(["tcp://192168.1.321", "tcp://192168.1.321:INVALID"])(
      "invalid path",
      async (path) => {
        await expect(
          Adapter.create(
          ...
          "Invalid TCP path, expected format: tcp://<host>:<port>",
        );
      },
    );
    ...
    it("invalid adapter", async () => {
      await expect(
        ...
        `Cannot discover TCP adapters at this time. Specify valid 'adapter' and 'port' in your configuration.`,
      );
    });
  ...
  describe("USB discovery", () => {
    ...
    describe("without config", () => {
      ...
      it("detects with conflict vendor+product IDs", async () => {
        ...
        listSpy.mockReturnValueOnce([
          { ...ZSTACK_ZBDONGLE_P, path: "/dev/ttyACM0" },
        ]);
        ...
        listSpy.mockReturnValueOnce([
          {
            ...ZSTACK_ZBDONGLE_P,
            path: "/dev/ttyACM0",
            pnpId: ZSTACK_ZBDONGLE_P.path.replace("/dev/serial/by-id/", ""),
          },
        ]);
    ...
    describe("with adapter+path config", () => {
      it("detects each adapter", async () => {
        ...
        adapter = await Adapter.create(
          { panID: 0x1a62, channelList: [11] },
          { adapter: "blz", path: BLZ_THIRDREALITY.path },
          "test.db.backup",
          {
            disableLED: false,
          },
      ...
      it("detects with pnpId instead of path", async () => {
        ...
        const adapter = await Adapter.create(
          { panID: 0x1a62, channelList: [11] },
          { adapter: "zboss", path: "/dev/ttyUSB0" },
          "test.db.backup",
          {
            disableLED: false,
          },
      ...
      it("uses default serialPortOptions of adapter", async () => {
        ...
        const adapter = await Adapter.create(
          { panID: 0x1a62, channelList: [11] },
          { adapter: "ember", path: "/dev/ttyUSB0" },
          "test.db.backup",
          {
            disableLED: false,
          },
      ...
      it("detects with conflict vendor+product IDs", async () => {
        ...
        adapter = await Adapter.create(
          { panID: 0x1a62, channelList: [11] },
          { adapter: "zstack", path: "/dev/ttyACM0" },
          "test.db.backup",
          {
            disableLED: false,
          },
      ...
      it("returns instance anyway on failure to match", async () => {
        ...
        const adapter = await Adapter.create(
          { panID: 0x1a62, channelList: [11] },
          { adapter: "zstack", path: "dev/ttyUSB0" },
          "test.db.backup",
          {
            disableLED: false,
          },
      ...
      it("throws on failure to match invalid adapter", async () => {
        ...
        await expect(
          ...
          `Adapter 'invalid' does not exists, possible options: zstack, ember, deconz, zigate, zboss, zoh, blz, ezsp`,
        );
      });
    });
    ...
    it("throws on failure to match when port info too limited", async () => {
      listSpy.mockReturnValueOnce([
        { ...DECONZ_CONBEE_II, vendorId: undefined },
      ]);
      ...
      listSpy.mockReturnValueOnce([
        { ...DECONZ_CONBEE_II, productId: undefined },
      ]);
